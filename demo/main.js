...
window.getBrowserMode = () => (window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll("[data-theme-mode]").forEach((el) => {
    el.addEventListener("click", () => {
      ...
      let currentMode                          = themeMode === "auto" ? getBrowserMode() : themeMode
      document.documentElement.dataset.bsTheme = currentMode
      document.querySelectorAll("[data-theme-mode]").forEach((el2) => { el2.querySelector(".mode-check").classList.toggle("d-none", el2 !== el) })
    })
  })
